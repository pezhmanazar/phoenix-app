...
router.get("/active", async (req, res) => {
  try {
    ...
    const active = await prisma.announcement.findMany({
      ...
      take: 10,
      select: {
        id: true, title: true, message: true, level: true, placement: true,
        dismissible: true, enabled: true, startAt: true, endAt: true,
        priority: true, createdAt: true, updatedAt: true,
      },
    });

    if (!phone) return res.json({ ok: true, data: active });

    const user = await prisma.user.findUnique({ where: { phone }, select: { id: true } });
    if (!user) return res.json({ ok: true, data: active });
    ...
    const filtered = active.filter((a) => (a.dismissible ? true : !seenSet.has(a.id)));
