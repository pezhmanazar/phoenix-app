...
function buildDiagramsAndSummary(answersJson, didSkipTest2) {
  ...
  const t1Diagrams = [
    {
      key: "t1_redflags",
      title: " خط قرمزهای رابطه",
      percent: redPercent,
      label: percentLabel(redPercent),
    },
    {
      key: "t1_satisfaction",
  ...
  // --- Safety/Quality signals (NEW) ---
  const relationshipDanger =
    redPercent >= 60 || (conflictPercent != null && conflictPercent >= 70);
  ...
  // ------------------ Part A: Relationship summary (ALWAYS) ------------------
  let relOne = "";
  let relStep = "";

  if (relationshipDanger) {
    relOne =
      "جمع‌بندی رابطه: این رابطه برای تو «ناایمن و آسیب‌زا» بوده یعنی حتی اگه برگشتی هم اتفاق بیفته یا خودت تلاش کنی به این رابطه برگردی، احتمال تکرار آسیب‌های وارد خیلی بالاست و اینجا مسئله دلتنگی نیست؛ مسئله امنیت روانی و مرزهای توئه.";
    relStep =
  ...
  } else if (personalDanger) {
    relOne =
      "جمع‌بندی رابطه: این رابطه برای تو «روان‌فرسا» بوده؛ یعنی وابستگی و فرسودگی آنقدر بالا رفته که ذهن و بدن تو رو در حالت اضطراب و قفل نگه داشته.";
    relStep =
      "قدم بعدی: همین الان درمان رو شروع کن، بدون شک بهترین مسیر برای تو همینه و حداقل تا پایان مرحله سوم درمان جلو برو تا سیستم عصبیت آروم بشه و تصمیم‌ها از حالت احساسی خارج بشه.";
  } else if (relationshipPoor) {
    relOne =
      "جمع‌بندی رابطه: رابطه‌ای که داشتی یک رابطه «متزلزل» بوده یعنی نه کاملاً امن بوده و نه کاملاً نابود. احتمالاً ترکیبی از لحظات خوب و فشارهای تکرارشونده بوده و همین باعث گیجی و دو‌دلی تو شده.";
    relStep =
      "قدم بعدی: همین الان درمان رو شروع کن، بدون شک بهترین مسیر برای تو همینه و حداقل تا پایان مرحله سوم درمان رو ادامه بده تا تمام مشکلاتی که بخاطر جدایی تحمل می‌کنی حل بشه.";
  } else if (relationshipGood) {
    relOne =
      "جمع‌بندی رابطه: از نظر کیفیت کلی، این رابطه نشانه‌های «نسبتاً سالمی» داشته (یعنی رضایت بالاتر، تعارض قابل‌مدیریت‌تر و وابستگی کمتر). با این حال تصمیم نهایی فقط با واقعیتِ رفتارهای بعد از جدایی مشخص میشه واگه الان این رابطه به اتمام رسیده باید دقیق‌تر همه چیز بررسی بشه.";
    relStep =
      "قدم بعدی: درمان رو شروع کن و حداقل تا پایان مرحله سوم برو. این کار هم انتخابت رو شفاف‌تر می‌کنه، هم چشمانت رو به حقیقت بازتر می‌کنه و هم اگه قرار باشه برگشت واقعی رخ بده، معمولاً احتمال برگشتن بیشتر می‌شه.";
  } else {
    relOne =
      "جمع‌بندی رابطه: داده‌ها برای یک حکم قطعی کافی نیست، اما یک اصل ثابت هست: تصمیم درست روی «واقعیتِ رابطه و رفتارها» ساخته می‌شه نه روی امید. پس وقتی فشار عاطفی بالاست، مغز نشانه‌های کوچیک رو بزرگ می‌کنه.";
    relStep =
      "قدم بعدی: درمان رو شروع کن و حداقل تا پایان مرحله سوم اون رو ادامه بده تا ذهن از حالت اضطرار بیرون بیاد. بعدش با معیارهای روشن تصمیم بگیر.";
  }

  // ------------------ Part B: Return summary (ONLY if Test2 exists) ------------------
  let retOne = "";
  let retStep = "";
  ...
  if (hasTest2) {
    ...
    if (goodReturn) {
      retOne =
        "احتمال برگشت: چند نشونه‌ی امیدوارکننده وجود داره، اما فقط اگه «پایدار و قابل اندازه‌گیری» باشه باید بهش توجه کنی.";
      retStep =
        "اگه برگشتن قراره واقعا اتفاق بیفته، باید سه شرط رو ببینی: ۱) تعهد روشن ۲) اقدام واقعی و پیوسته ۳) احترام کامل به مرزهای تو. و با وجود نشونه‌های خوب، مسیر درمان رو حداقل تا پایان مرحله سوم ادامه بده.";
    } else {
      retOne =
        "احتمال برگشت: تصویر غالبی که الان از رابطه تو وجود داره یک ابهام بزرگه؛ یعنی احتمالاً تو در حالت انتظار نگه داشته شدی یا اینکه پارتنر سابق تو هیچ تمایلی به برگشت نداره چون نشونه‌های بازگشت خیلی کمه، پس به جزئیات تحلیلی این آزمون که در پایین صفحه اومده دقت کن تا به طور کامل متوجه بشی.";
      retStep =
        "این قانون ساده رو بدون که «بدون اقدام مشخص، حرف ارزش نداره.» پس مسیر درمان رو حداقل تا پایان مرحله سوم ادامه بده. اگه تا آن موقع اقدام واقعی ندیدی، این رابطه رو تموم‌شده فرض کن و درمان رو تا پایان ادامه بده.";
    }
  }

  // ------------------ Compose final text (ALWAYS relationship first) ------------------
  oneLook = relOne + (hasTest2 ? "\n\n" + retOne : "");
  nextStep = relStep + (hasTest2 ? "\n\n" + retStep : "");
  ...
  return {
    ...
    summary: {
      ...
      nextStep,
      meta: {
        // ✅ meta کامل برای دیباگ/تصمیم‌سازی اپ
        signals: {
          relationshipDanger,
          personalDanger,
          relationshipPoor,
