...
async function seedGosastanTasksDay5() {
  const day = await prisma.pelekanDay.findUnique({
    where: { globalDayNumber: 13 },
  });

  if (!day) {
    throw new Error('Gosastan day 5 not found. Please run seed-gosastan-days first.');
  }
  ...
  console.log('✅ Gosastan day 5 tasks seeded successfully.');
}

module.exports = seedGosastanTasksDay5;

if (require.main === module) {
  seedGosastanTasksDay5()
    .catch((error) => {
      console.error('❌ Error seeding gosastan day 5 tasks:', error);
      process.exit(1);
    })
    .finally(async () => {
