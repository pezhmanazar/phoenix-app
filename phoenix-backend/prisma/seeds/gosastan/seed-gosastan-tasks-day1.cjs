...
async function seedGosastanTasksDay1() {
  const day = await prisma.pelekanDay.findUnique({
    where: { globalDayNumber: 9 },
  });

  if (!day) {
    throw new Error('Gosastan day 1 not found. Please run seed-gosastan-days first.');
  }
  ...
  console.log('✅ Gosastan day 1 tasks seeded successfully.');
}

module.exports = seedGosastanTasksDay1;

if (require.main === module) {
  seedGosastanTasksDay1()
    .catch((error) => {
      console.error('❌ Error seeding gosastan day 1 tasks:', error);
      process.exit(1);
    })
    .finally(async () => {
