...
// ✅ پرداخت: وب‌براوزر + لینکینگ + ریدایرکت
...
// همان کلیدی که در تب پلکان استفاده می‌شود
...
const [localPlan, setLocalPlan] = useState<"free" | "pro" | null>(null); // UI خوش‌بینانه
const [proFlag, setProFlag] = useState(false);                  // فلگ لوکال مثل پلکان
...
try {
  ...
  // deep link callback => phoenixapp://pay
  ...
  const sub = LinkingExpo.addEventListener("url", async (ev) => {
    try {
      ...
      const planResp = ((ver as any).plan ||
        (ver as any).data?.plan ||
        "pro") as "pro" | "free";
      // ✅ UI خوش‌بینانه + ست فلگ لوکال
      setLocalPlan(planResp);
      if (planResp === "pro") {
        setProFlag(true);
        await AsyncStorage.setItem(PRO_FLAG_KEY, "1").catch(() => {});
      }
      console.log(
        "PHOENIX PAY SUCCESS planResp =",
        planResp,
        " (قبل از refresh)"
      );
      Alert.alert("پرداخت موفق", `کد رهگیری:\n${refId}`);
      // رفرش از سرور / کانتکست
      await refresh().catch(() => {});
    } finally {
      sub.remove();
      payingRef.current = false;
...
// رفرش یک‌بار هنگام ورود
useEffect(() => {
refresh().catch(() => {});
// eslint-disable-next-line react-hooks/exhaustive-deps
}, []);
// خواندن فلگ پرو از استوریج (مثل تب پلکان)
useEffect(() => {
(async () => {
  try {
...
// هر بار فوکوس تب، اطلاعات کاربر را از سرور تازه کن
useFocusEffect(
useCallback(() => {
  console.log("PHOENIX FOCUS - calling refresh()");
  refresh().catch(() => {});
  return () => {};
}, [refresh])
);
// اگر سرور نام/آواتار دارد، UI محلی را هم‌سطح کن
useEffect(() => {
...
// بستن مودال در آن‌ماینت صفحه (ایمنی)
useEffect(() => {
...
// ✅ این مقدار تعیین‌کننده‌ی نمایـش پلن در UI است
let effectivePlan: "free" | "pro" = "free";
// ۱) سرور
if ((me?.plan as any) === "pro") effectivePlan = "pro";
// ۲) فلگ لوکال (paytest یا هر جایی که phoenix_is_pro را ست می‌کند)
if (proFlag) effectivePlan = "pro";
// ۳) حالت خوش‌بینانه بعد از پرداخت آنلاین همین تب
if (localPlan === "pro") effectivePlan = "pro";
console.log(
"PHOENIX RENDER me.plan =",
me?.plan,
"proFlag =",
proFlag,
"localPlan =",
...
useFocusEffect(
useCallback(() => {
  // هر بار تب ققنوس فوکوس می‌گیرد، فلگ را دوباره از استوریج بخوان
  (async () => {
    try {
      const v = await AsyncStorage.getItem(PRO_FLAG_KEY);
      const isPro = v === "1";
      setProFlag(isPro);
      console.log("PHOENIX FOCUS FLAG FROM STORAGE =", v, "=>", isPro);
    } catch {
      setProFlag(false);
    }
  })();
  // localPlan فقط برای همان لحظه بعد از پرداخت است؛
  // روی هر فوکوس، خنثی‌اش کن تا به سرور/فلگ اعتماد کنیم
  setLocalPlan(null);
  console.log(
    "PHOENIX FOCUS me.plan =",
    me?.plan,
    "proFlag =",
    proFlag,
    "localPlan (بعد از reset) =",
    null
  );
  return () => {};
}, [me?.plan])
...
<View style={{ flex: 1, backgroundColor: colors.background }}>
  ...
  {/* رندر شرطی مودال — بدون prop visible */}
