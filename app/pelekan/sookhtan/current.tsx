// app/pelekan/sookhtan/current.tsx
...
const RESET_DAY_URL = "https://qoqnoos.app/api/pelekan/stage/sookhtan/day/reset";
...
export default function SookhtanCurrentScreen() {
  const insets = useSafeAreaInsets();
  const router = useRouter();
  const { dayNumber } = useLocalSearchParams<{ dayNumber?: string }>();
  const safeDayNumber = Number(dayNumber || 1);
  const API_URL = `https://qoqnoos.app/api/pelekan/stage/sookhtan/day/${safeDayNumber}/state`;
  ...
  const [data, setData] = useState<SookhtanStateResponse | null>(null);
  ...
  const fetchState = useCallback(
    async (isPullToRefresh = false) => {
      ...
      try {
        ...
        const json: SookhtanStateResponse = await res.json().catch(() => ({
          ok: false,
          error: "INVALID_JSON",
        }));
        ...
        setError(e?.message || "در دریافت اطلاعات سوختن خطایی رخ داد.");
      } finally {
        setLoading(false);
        setRefreshing(false);
      }
...
const handleResetDay = async () => {
  ...
  const dayDescription =
    ...
    "امروز با آگاهی وارد آتش تغییر می‌شی؛ آروم، صادقانه و بدون فرار.";
      ...
      >
        ...
        <View style={styles.centerBox}>
          ...
          <Text style={[styles.errorText, { color: palette.sub }]}>
            برای مشاهده روز جاری سوختن، اول وارد حساب کاربریت شو.
          </Text>
            ...
            {tasks.map((task, index) => {
              ...
              return (
                <View key={String(task.id)} style={styles.taskBlock}>
                  <Link
  href={{
    pathname: "/pelekan/sookhtan/[code]",
    params: {
